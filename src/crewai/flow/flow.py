...
class Flow(Generic[T], metaclass=FlowMeta):
    ...
    def _create_initial_state(self) -> T:
        ...
        if isinstance(self.initial_state, dict):
            ...
            return cast(
                T, dict(self.initial_state)
            )  # Create new dict to avoid mutations
    ...
    def _find_triggered_methods(
        ...
        """
        triggered = []
        pending_and_listeners = self._pending_and_listeners
        routers = self._routers
        listeners_items = self._listeners.items()

        for listener_name, (condition_type, methods) in listeners_items:
            is_router = listener_name in routers
            ...
            if condition_type == "OR" and trigger_method in methods:
                ...
                pending_methods = pending_and_listeners.setdefault(
                    listener_name, set(methods)
                )

                if trigger_method in pending_methods:
                    pending_methods.discard(trigger_method)

                if not pending_methods:
                    triggered.append(listener_name)
                    pending_and_listeners.pop(listener_name, None)
